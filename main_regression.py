...
unique_class_numbers = list(set(np.array(train_loader.dataset.styles))) #
...
lb, ub = min(years), max(years) #get lower and upper bound
# Initialize the polar bound polar.
prototype_years = np.zeros(output_dimension)
prototype_years[0] = 1

normalized_years = 2. * ((years - lb) / (ub - lb)) - 1 #normalize the years between the lower and upper bound
...
for epoch in range(1, epochs + 1):
    if epoch%100 == 0:
        lr = lr/10
        optimizer = optim.SGD(model.parameters(), lr=lr, momentum=momentum, weight_decay=1e-4)
    train(model, device, train_loader, optimizer, epoch, classification_matched_points = classification_matched_points)
    test(model, device, test_loader, classification_matched_points, epoch)
