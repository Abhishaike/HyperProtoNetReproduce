from nn_losses import get_classification_loss, get_regression_loss, get_joint_loss
import torch
...
def train(model, device, train_loader, optimizer, epoch, classification_matched_points, operation, upper_bound, class_weight = .5):
    model.train()
    for batch_idx, (data, local_labels_year, local_labels_style) in enumerate(train_loader):
        ...
        if operation == 'regression':
            total_loss = get_regression_loss(local_labels_year, hypersphere_prediction, upper_bound)
            ...
            total_loss = get_joint_loss(hypersphere_prediction, prototype_classification)
        total_loss.backward()
        optimizer.step()
        ...
        if batch_idx%50 == 0:
            ...
            correct = (np.array(hypersphere_labels) == np.array(local_labels_style.cpu())) * 1
            print('Style train Accuracy: ', correct.mean())

        #cv2.imshow('hi',image[5].cpu().numpy().transpose(1, 2, 0)) #see example batch image


def test(model, device, test_loader, classification_matched_points, epoch, objective, upper_bound, lower_bound):
    model.eval()
    all_correct = []
    all_loss = []
    with torch.no_grad():
        for batch_idx, (local_batch, local_labels) in enumerate(test_loader):
            prototype_classification = torch.FloatTensor([list(classification_matched_points[class_num.item()]) for class_num in local_labels]).to(device)  # get the vector of the label
            image, hypersphere_labels = local_batch.to(device), local_labels.to(device)
            hypersphere_prediction = model(image)
            classification_loss = classification_loss(hypersphere_prediction, prototype_classification)
            pred_labels = assign_predicted_class(hypersphere_prediction, classification_matched_points) #get closest matching prototypes
